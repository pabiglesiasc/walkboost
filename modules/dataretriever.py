...
def get_data(
    ...
    if custom_stocks!='':
        custom_tickers = pd.Series(custom_stocks.split(';'))
    ...
    data_ohlcv = tickers.history(start=start_date, end=end_date)
    ...
    if macroeconomic:
        ...
        data_macroeconomic = yf.Tickers(macroeconomic_indicators).history(start='1991-01-01', end='2023-12-31')
    ...
    if correlated:
        ...
        corr_stocks = data_price.columns[data_price.notna().sum().gt(252*trading_years)]
        data_corr = data_price.corr().loc[corr_stocks]
        data_corr = data_corr.apply(lambda x: x.nlargest(number_correlated+1).index.tolist()[1:]).T

        def get_price_corr(data_price, x, stock):

            data_price_corr = data_price[x]
            data_price_corr.columns = [f'Corr{i+1}_Close' for i in range(number_correlated)]
        ...
        stocks = trading_days[trading_days.gt(252*trading_years)].index
